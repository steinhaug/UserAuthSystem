...
router.get('/check', (req, res) => {
  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).json({ 
      status: 'error', 
      message: 'Gemini API key not configured. Please add it to your environment variables.' 
    });
  }
...
// Function to convert an object to a pretty-printed string for model prompts
function objectToPrettyString(obj: any): string {
  return JSON.stringify(obj, null, 2);
}

// Helper function for creating a standard Gemini model configuration
function createGeminiModel(modelName = 'gemini-1.5-pro') {
...
// Note: Gemini doesn't have native audio transcription like OpenAI's Whisper
...
router.post('/transcribe', upload.single('audio'), async (req, res) => {
  try {
    ...
    // Since Gemini doesn't have a native audio transcription API like Whisper,
    // we return a fallback message. In a production app, you would implement
    // integration with another speech-to-text service like Google Speech-to-Text.
    ...
    return res.json({ 
      text: "Stemmegjenkjenning er for øyeblikket utilgjengelig med Gemini API. Vennligst skriv søket ditt i stedet.",
      fallback: true 
    });
...
// Helper function to provide basic search results without Gemini
...
// Endpoint for location search using Gemini
router.post('/location-search', async (req, res) => {
  try {
    ...
    // Check if Gemini API key is properly configured
    if (!process.env.GEMINI_API_KEY) {
      console.log('Gemini API key not properly configured. Using fallback search functionality.');
    ...
    try {
      ...
      const prompt = `
You are a location search assistant that helps find places based on user queries. 
For the query: "${query}"

Return a JSON object with an array of places. Each place should have:
...
router.post('/location-suggestions', async (req, res) => {
  try {
    ...
    // Check if Gemini API key is properly configured
    if (!process.env.GEMINI_API_KEY) {
      console.log('Gemini API key not properly configured. Using fallback suggestions.');
    ...
    try {
      ...
      const prompt = `
You are a local guide assistant that suggests interesting places near a user's location.
...
`;
      ...
      console.error('Gemini API error:', geminiError);
      return res.status(500).json({ error: 'Failed to generate suggestions from Gemini' });
    }
  } catch (error) {
